...
abstract class Sharing {
	...
	public function maybe_extend_doc_head( $head_contents ) {
		...
		if ( 'replace' === $this->data['sharing_tag_insert_mode'] ) {
			$replace_result = $this->remove_tags( $head_contents, $tag_identifiers );
			$head_contents  = $replace_result['content'];
			$replaced_tags  = $replace_result['replaced_tags'];
		}

		$tag_html = $this->get_tag_html( $tags, $block_info, $replaced_tags );
	 ...
	 *
	 * @return mixed[] Content string with the tags removed and related tag names.
	 */
	protected function remove_tags( $content, $tags ) {
		...
		foreach ( $tags as $tag ) {
			...
			if ( $count ) {
				$replaced_tags[] = preg_replace( '/.*?"(.*?)".*/', '$1', $tag );
			}
		}

		if ( $add_comments && ! empty( $replaced_tags ) ) {
	 ...
	 * @param string[] $replaced_tags Names of replaced tags (optional).
	 *
	 * @return string Tag HTML block.
	 */
	protected function get_tag_html( $tags, $html_comment, $replaced_tags = array() ) {
	...
	/**
	 * Get property post or archive title.
	 *
	 * @since 1.11.12-beta
	 *
	 * @param int|string|bool $page_post_id Page/Post ID or false if archive.
	 *
	 * @return string Post/Archive title.
	 */
	protected function get_title( $page_post_id ) {
		$title = $page_post_id ? get_the_title( $page_post_id ) : get_the_archive_title();

		if ( static::TITLE_MAX_LENGTH && strlen( $title ) > static::TITLE_MAX_LENGTH ) {
			return $this->utils['string']->get_excerpt( $title, static::TITLE_MAX_LENGTH, '…' );
		}
	...
	/**
	 * Get property post or archive excerpt/description.
	 *
	 * @since 1.11.12-beta
	 *
	 * @param int|string|bool $page_post_id Page/Post ID or false if archive.
	 *
	 * @return string Post/Archive description.
	 */
	protected function get_description( $page_post_id ) {
		$excerpt = $page_post_id ? get_the_excerpt( $page_post_id ) : get_the_archive_description();
		$excerpt = wp_strip_all_tags( preg_replace( '/<a[^>]*>.*?(<\/a>|$)/', '', $excerpt ) );

		if ( static::DESCRIPTION_MAX_LENGTH && strlen( $excerpt ) > static::DESCRIPTION_MAX_LENGTH ) {
			return $this->utils['string']->get_excerpt( $excerpt, static::DESCRIPTION_MAX_LENGTH, '…' );
		}
