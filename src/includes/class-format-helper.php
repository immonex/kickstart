...
class Format_Helper {
	...
	public function get_heading_tag( $level, $close = false, $classes = '' ) {
		$level += $this->config['heading_base_level'] - 1;
		if ( $level > 6 ) {
			$level = 6;
		}
	...
	public function format_price( $value, $decimals = 2, $price_time_unit = '', $if_zero = '', $currency = '' ) {
		...
		$currency = ! empty( $currency ) ? $currency : $this->config['currency_symbol'];
	...
	public function format_area( $value, $decimals = 2, $if_zero = '', $unit = '' ) {
		...
		$unit     = ! empty( $unit ) ? $unit : $this->config['area_unit'];
		$decimals = 9 === $decimals ? 99 : $decimals;
		$args     = array(
			'if_zero'  => $if_zero,
			'unit_sep' => '&nbsp;',
	...
	public function get_json_ld_script_block( $schema_data, $add_info = false ): string {
		...
		if ( $add_info ) {
			$info = wp_sprintf(
				...
				__( 'Structured data (Schema.org) generated by immonex® Kickstart %1$s – Real Estate Plugin (Open Source) – %2$s', 'immonex-kickstart' ),
